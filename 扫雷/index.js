function Mine(tr,td,mineNum){
    ...
    this.squares=[];  //存储所有方块的信息，他是一个二维数组，按行与列的顺序排放。存取都使用行列形式
...
Mine.prototype.init=function(){
    ...
    for(var i=0;i<this.tr;i++){
        ...
        for(var j=0;j<this.td;j++){
            ...
            //取一个方块在数组里的数据要使用行与列的形式去取。找方块周围的方块的时候要用坐标的形式去取，行与列的形式和坐标形式x，y是刚好相反的
            if(rn.indexOf(++n)!=-1){
                //如果这个条件成立，说明现在循环到这个索引在累的数组里找到了，那就表示这个索引对应的是个雷
...
Mine.prototype.play=function(ev,obj){
    var This=this;
    if(ev.which==1&&obj.className!='flag'){  //后面的条件是为了限制用户在标了小红旗之后就不能再左击了
        //点击的是左键
        //console.log(obj);
                    ...
                    for(var i=0;i<around.length;i++){
                        ...
                        if(This.squares[x][y].value==0){
                            ...
                            if(!This.tds[x][y].check){
                                //给对应的td添加一个属性，这条属性用于决定这个格子有没有被找过，如果找过的话，他的值就位true，下次就不会再被找了
    ...
    if(ev.which==3){
        ...
        if(this.surplusMine==0){
            //剩余的雷的数量为0，表示用户已经标完小红旗了，这时需要判断游戏是成功还是失败
            if(this.allRight){
                //这个条件成立表示用户全部标对了
                alert('兄弟，恭喜你，游戏通过了！');
            }else{
